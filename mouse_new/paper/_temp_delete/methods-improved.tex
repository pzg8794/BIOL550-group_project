...
\node[stage, fill=lightmine, draw=stagemine, right=of s3] (s4)
  {\textcolor{stagemine}{\textbf{Data Analysis \& Interpretation}}\\
...
\caption{End-to-end structure of the \texttt{mouse\_new} RNA-seq workflow organized into four
  reproducible computational stages.  The Data Analysis stage encompasses five DESeq2 contrasts
...
\texttt{SRP618841} (BioProject \texttt{PRJNA1322439}; GEO \texttt{GSE243308}).
...
The working subset consisted of 20 DRG samples generated on a NovaSeq X platform under a sciatic
nerve injury (SNI) design, all sampled at 1 day post-injury (1~dpi).
...
\caption{Per-sample design table for the \texttt{family\_drg\_novaseqx} subset
  ...
  All samples: tissue = DRG, treatment = Sciatic Nerve Injury 1\,dpi, platform = NovaSeq X,
...
Reconstruct the working DRG subset locally from public data &
  BioProject \texttt{PRJNA1322439}; GEO \texttt{GSE243308};
...
\caption{Data Collection stage summary figure. The panel shows the public accession lineage
  (\texttt{SRP618841} / \texttt{PRJNA1322439} / \texttt{GSE243308}), the local acquisition
...
\centering
\small
\caption{Data Analysis and Interpretation stage summary. Rows are ordered by contrast priority
...
\caption{Data Analysis and Interpretation stage summary derived from the saved \texttt{DESeq2},
