...
# End-to-end runner (parallel raw stage):
#   download/convert (SRA) -> FastQC (raw) -> FASTX trim -> FastQC (trimmed)
#
# Diff vs run_end_to_end_fastq_fastqc_fastx_fastqc.sh:
#   - Uses sra_runs_pipeline_sra3_parallel.sh for Stage 1 (multiple workers)
#   - Adds DOWNLOAD_WORKERS / FASTQC_WORKERS / PIGZ_THREADS knobs (Stage 1)
...
SRA_PIPE="$SCRIPT_DIR/sra_runs_pipeline_sra3_parallel.sh"
...
count_expected() { awk 'NF && $1 !~ /^#/{print $1}' "$RUNS_FILE" | wc -l | tr -d ' '; }

progress_raw() {
  local expected; expected="$(count_expected)"
...
wait_for_file() {
  local f="$1" label="$2" sleep_s="${3:-60}"
  while [[ ! -f "$f" ]]; do log "waiting: $label"; sleep "$sleep_s"; done
}
...
run_end_to_end() {
  ...
  log "params: DOWNLOAD_WORKERS=$DOWNLOAD_WORKERS FASTQC_WORKERS=$FASTQC_WORKERS PIGZ_THREADS=$PIGZ_THREADS DUMP_THREADS=$DUMP_THREADS FASTQC_THREADS_RAW=$FASTQC_THREADS_RAW FASTQC_THREADS_TRIM=$FASTQC_THREADS_TRIM TRIM_QUAL=$TRIM_QUAL MIN_LEN=$MIN_LEN"

  log "== STAGE 1: download + FastQC (raw; parallel) =="
  ACC="$ACC" MEMBER="$MEMBER" RUNS_FILE="$RUNS_FILE" \
  SHARED_RUN_DIR="$SHARED_RUN_DIR" FASTQC_OUT="$FASTQC_OUT_RAW" PIPE_DIR="$PIPE_RAW" \
  DOWNLOAD_WORKERS="$DOWNLOAD_WORKERS" FASTQC_WORKERS="$FASTQC_WORKERS" PIGZ_THREADS="$PIGZ_THREADS" \
...
Notes:
  - Stage 1 runs with multiple workers (DOWNLOAD_WORKERS / FASTQC_WORKERS).
  - Logs live under: \$DATA_ROOT/.pipeline/
EOF
     exit 2
     ;;
esac
