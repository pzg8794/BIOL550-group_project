...
## 2026-04-16 — April 16 ShinyGO weekly report problems/goals sections tightened

### Step
- Revised only the `Problems encountered` and `Goals for the coming week` sections in:
  - `mouse_new/reports/BIOL550_Weekly_Report_Mouse_DE_ShinyGO_2026-04-16_v3_updated.html`

### Status
- The updated report now frames those sections around reader-facing clarity instead of repo logistics or paper-admin language.

### Finding
- The earlier second problems paragraph was too operational and did not read like a real reporting challenge.
- The goals section became stronger once it focused on figure selection, non-specialist readability, and explicit regeneration / injury-response framing.

### Decision
- Keep this report version as the stronger April 16 ShinyGO weekly-report draft unless a later team-facing wording pass is needed.
...
## 2026-04-16 — Results section restored and GO methods/context added to paper draft

### Step
- Added a full `Results` section to:
  - `Semester5/BIOL550/group_project/mouse_new/paper/materials_methods_piter_draft.tex`
- Built the Results section around notebook-derived figures already staged under `mouse_new/paper/assets_htsa/` plus the retained FF / CRE network views from the GO follow-up outputs.
- Added a concise reader-facing methods paragraph for the GO follow-up using the same logic from the previous weekly report:
  - `DESeq2` notebook for contrast modeling and bend-point selection
  - `g:Profiler` + ShinyGO-style local notebook for chart/tree/network/membership follow-up
- Moved the FF-versus-CRE comparison paragraph out of the early Discussion flow and placed it after the paired GO network plots, where it reads as a Results-supported comparison instead of a premature interpretation block.
- Expanded `HTSA_Paper_Asset_Inventory.md` so the current manuscript-ready Results figure set and shareable supporting tables are easier to hand to teammates.

### Finding
- The draft flow was still broken because it jumped from Methods into Discussion without a Results evidence block.
- The branch-comparison paragraph read awkwardly inside Discussion before the reader had seen the paired network figures.
- The earlier weekly reports already contained a cleaner explanation of the methods used for the GO follow-up than the paper draft did.

### Decision
- Keep the Results section centered on the strongest manuscript-ready figures already generated from the notebooks: QC, alignment, PCA/heatmap, bend-point narrowing, GO composite, and paired WT/cKO network views.
- Keep the GO methods explanation concise and anchored to the actual notebooks and transcript guidance rather than expanding Methods again inside Discussion.

## 2026-04-16 — April 16 ShinyGO weekly report tone and content refinement

### Step
- Revised the `Problems encountered` and `Goals for the coming week` sections in:
  - `Semester5/BIOL550/group_project/mouse_new/reports/BIOL550_Weekly_Report_Mouse_DE_ShinyGO_2026-04-16_v3_updated.html`
- Reshaped the goals section to match the earlier weekly-report pattern more closely:
  - one explanatory paragraph
  - a shorter supporting bullet list
- Added one notebook-grounded interpretive paragraph from the FF ShinyGO follow-up notebook linking the retained GO themes back to recurring anchor/support genes (`Atf3`, `Gadd45a`, `Jun`, `Sox11`, `Flrt3`).

### Finding
- The previous revision was still too list-heavy and too generic in tone compared with the April 2 and April 9 reports.
- The strongest notebook-derived addition was not another figure, but a compact gene-support explanation that makes the retained GO themes easier to justify biologically.
- The notebook confirms that the cleanest additional material is the bridge between enriched GO labels and specific supporting FF genes, which fits the open space better than adding another dense panel.

### Decision
- Keep the April 16 report focused on a small number of explainable GO outputs and gene-supported themes rather than expanding the figure count.
- Use the chart-view-plus-gene-support direction as the most paper-usable continuation from this report.

## 2026-04-16 — April 16 ShinyGO weekly report Methods section and layout fix

### Step
- Repaired the structure of:
  - `Semester5/BIOL550/group_project/mouse_new/reports/BIOL550_Weekly_Report_Mouse_DE_ShinyGO_2026-04-16_v3_updated.html`
- Removed the misplaced notebook-derived anchor-gene paragraph from the left summary column.
- Reinserted that interpretive paragraph after the visual row so it now reads as a bridge from the network figure(s) into the text.
- Added a new `Methods used` section in the style of the earlier weekly reports, using transcript and notebook context to describe:
  - bend-point contrast reuse
  - g:Profiler reruns with retained term membership
  - ShinyGO-style redundancy reduction
  - FDR- and explainability-driven figure/term selection
  - anchor-gene tracing from term-membership outputs

### Finding
- The report was missing the expected `Methods used` section entirely, which broke continuity with the earlier weekly reports.
- The added gene-support paragraph was useful content, but its placement inside the table column made the page look structurally wrong and weakened the flow into the later sections.
- The transcript-guided method story is strongest when it emphasizes selective, explainable GO reporting rather than just listing software steps.

### Decision
- Keep the anchor-gene interpretation after the figure row, where it functions as a results bridge rather than a table note.
- Keep `Methods used` as compact prose tied to this week’s actual GO follow-up workflow and class guidance, instead of reverting to a generic RNA-seq pipeline summary.

## 2026-04-20 — HTSA_Paper cleanup and Overleaf sync

### Step
- Fixed the current `mouse_new/paper/HTSA_Paper.tex` so the active LaTeX source compiles cleanly again.
- Rebuilt the canonical paper trio in place:
  - `mouse_new/paper/HTSA_Paper.tex`
  - `mouse_new/paper/HTSA_Paper.pdf`
  - `mouse_new/paper/HTSA_Paper.docx`
- Archived superseded `HTSA_Paper` one-off variants under:
  - `mouse_new/paper/HTSA_Paper_archive/historical_variants/`
- Moved `HTSA_Paper` LaTeX build byproducts under:
  - `mouse_new/paper/HTSA_Paper_archive/build_artifacts/`
- Added `mouse_new/paper/HTSA_Paper_LIFECYCLE.md` to document the canonical trio, archive location, and Overleaf sync target.
- Cloned and synchronized the paper package into:
  - `Semester5/BIOL550/BIOL550-Project_Paper`
  with `main.tex` mirrored from `HTSA_Paper.tex`, plus the active PDF/DOCX, bibliography, and `assets_htsa/` figure set.

### Finding
- The active `HTSA_Paper.tex` was newer than the top-level PDF/DOCX and also contained a duplicated `piterpara` opening in the Discussion section that had to be fixed before a clean rebuild.
- The top-level paper folder had accumulated many obsolete `HTSA_Paper_*` exports, which made it hard to tell which files were actually current.
- The Overleaf Git repo was not yet cloned locally and initially contained only a placeholder `main.tex`.

### Decision
- Treat the rebuilt `HTSA_Paper.tex` as the canonical source for the aligned trio after confirming it produced the newest matching PDF and DOCX.
- Keep only one active top-level `HTSA_Paper.tex` / `.pdf` / `.docx` trio in `mouse_new/paper` and archive the rest instead of leaving them mixed into the working surface.
- Keep the Overleaf repo synchronized with the current paper package while avoiding unrelated course artifacts.

## 2026-04-20 — Paper workspace consolidation into Overleaf repo

### Step
- Moved the full HTSA paper workspace out of `mouse_new/paper/` and into the dedicated Overleaf Git repository at `Semester5/BIOL550/BIOL550-Project_Paper`.
- Preserved the active submission trio at the Overleaf repo top level:
  - `HTSA_Paper.tex`
  - `HTSA_Paper.pdf`
  - `HTSA_Paper.docx`
- Organized supporting materials inside the Overleaf repo into internal groups:
  - `docs_internal/`
  - `archive/`
  - `assets_htsa/`
  - `assets_methods/`
  ...
  - `tmp_local/`
  - `env_local/`
- Left `mouse_new/paper/` as a clean pointer directory so the paper has a single real home.

### Finding
- The old `mouse_new/paper/` workspace had become a mixed staging area containing active paper files, archived exports, helper docs, assets, local build artifacts, temporary render outputs, and a local virtual environment.
- Keeping the paper split between `group_project` and the Overleaf repo created unnecessary duplication and made the canonical paper location ambiguous.

### Decision
- Treat `Semester5/BIOL550/BIOL550-Project_Paper` as the only real paper workspace going forward.
- Keep `mouse_new/paper/` clean and use it only as a pointer, not as a second drafting area.
- Organize materials only; do not revise paper content as part of this consolidation pass.

## 2026-04-20 — Weekly report deduplication cleanup

### Step
- Cleaned `mouse_new/reports/` so only one weekly report file remains per report week among the duplicated April deliverables.
- Kept the most recently modified report file for each duplicated week:
  - `fixed_mouse_de_weekly_report.pdf` for Apr 2, 2026
  - `BIOL550_Weekly_Report_Mouse_Differential_Expression_2026-04-09_GO_Methods.pdf` for Apr 9, 2026
  - `BIOL550_weekly_report_fixed_v3.pdf` for Apr 16, 2026
- Moved older duplicate report variants into `mouse_new/reports/archive_for_deletion/2026-04-20/` instead of deleting them directly.

### Finding
- The reports directory had multiple repeated HTML/PDF variants for the same week, generated under different naming schemes.
- Several files had newer filesystem timestamps only because they were regenerated later, so the cleanup was verified against the actual weekly report date/content before archiving.

### Decision
- Leave only one non-archived report artifact per duplicated week at the top level.
- Preserve superseded variants in an archive-for-deletion folder so they can be removed safely later after confirmation.

## 2026-04-20 — Final BIOL550 report/archive cleanup sweep

### Step
- Consolidated the report cleanup so `mouse_new/reports/` now uses a single top-level `archive/` folder instead of multiple archive-style directories.
- Moved `archive_duplicates/`, `archive_for_deletion/`, and `to_pdf/` content under the single `archive/` tree.
- Removed stray `.DS_Store` files across the BIOL550 directory tree.
- Kept only the active weekly report files at the top level of `mouse_new/reports/`, alongside the non-report support file and Nikhi’s report folder.

### Finding
- The directory still had multiple archive-like folders that all served the same cleanup purpose, which made the reports area look messier than necessary.
- Temporary conversion folders like `to_pdf/` were no longer useful once the duplicate weekly reports had been archived.

### Decision
- Use one archive location only: `mouse_new/reports/archive/`.
- Keep the active working surface minimal and move all stale report variants under that single archive tree.
- Keep `BIOL550-Project_Paper` as the single paper workspace and `mouse_new/paper/` as a pointer only.
